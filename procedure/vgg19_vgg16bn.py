""" average poolingとmax poolingのどっちがいいか？"""
import _parent
import pickle
import torch
import torch.nn as nn
import torch.optim as optim
from models.elastic_vgg16_batch_normalized import Elastic_VGG16_BN as vgg16bn
from models.elastic_vgg19 import Elastic_VGG19 as vgg19
...
parser.add_argument('--convolution',
                    choices=['vgg16bn_with_maxpool', 'vgg16bn_without_maxpool', 'vgg19_with_maxpool', 'vgg19_without_maxpool'],
...
MODEL_NAME = "{}_{}_{}_{}_{}_{}_hori_modif_vgg19_16bn".format(
...
if __name__ == '__main__':
    torch.manual_seed(0)
    if args.convolution == "vgg16bn_with_maxpool":
        model = vgg16bn(
            num_classes=10, model_type="vgg_with_maxpool", pooling=args.pooling,
            sync=args.fc, deepness=args.deepness, poolingshape=7
        )
    elif args.convolution == "vgg16bn_without_maxpool":
        model = vgg16bn(
            num_classes=10, model_type="vgg_without_maxpool", pooling=args.pooling,
            sync=args.fc, deepness=args.deepness, poolingshape=7
        )
    elif args.convolution == "vgg19_with_maxpool":
        model = vgg19(
            num_classes=10, model_type="vgg_with_maxpool", pooling=args.pooling,
            sync=args.fc, deepness=args.deepness, poolingshape=7
        )
    elif args.convolution == "vgg19_without_maxpool":
        model = vgg19(
            num_classes=10, model_type="vgg_without_maxpool", pooling=args.pooling,
            sync=args.fc, deepness=args.deepness, poolingshape=7
        )
