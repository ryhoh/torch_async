...
class Independent_Neuron_Rate_LinearFunction(Function):
    ...
    def backward(ctx,grad_out):
        ...
        # 次の層で使う変数の引き継ぎ
        global grad_b_send
        send_msk = grad_b.abs() > 0
        g_send = grad_b.masked_fill(send_msk, 1)
        grad_b_send = g_send

        return grad_x, grad_w, grad_b, None


class Dependent_Neuron_Rate_LinearFunction(Function):
    """従属％のLinearFunction
    更新するニューロンをレイヤ内からn％選ぶ(n=√n ,25, 50, 75, 100), またレイヤ間の従属関係がある
    更新するニューロンは重みとバイアスの勾配から決定する
    勾配の絶対値から大きな値を上位n％個選び更新する
    """
    @staticmethod
    ...
    def backward(ctx,grad_out):
        ...
        if 0 <= rate < 100:
            ...
            # 従属処理(grad_b_sendは前層の情報)
            g_dependent = grad_b_send
            g_w = g_w * g_dependent.unsqueeze(0)
            g_b = g_b * g_dependent
...
class Dependent_Neuron_Rate_Linear(Linear):
    """従属のLinear
    従属更新のレイヤー
    """
    def __init__(self, input_feautures, output_features, bias=True, rate=100):
        ...
        self.rate = rate
    def forward(self, input):
        res = Dependent_Neuron_Rate_LinearFunction.apply(input, self.weight, self.bias, self.rate)
        return res
...
if __name__ == "__main__":
    ...
    MODEL_NAME = "{}_{}_{}_dependent_neuron_rate".format(
    ...
    MODEL.classifier[3] = Dependent_Neuron_Rate_Linear(
