...
class ResNetForCifar():
    ...
    def __init__(self, layer_n: int = 56):
        """
        @param layer_n:
            The number of weighted layers.
            Must be (natural number * 6 + 2).
            e.g. 20, 32, 44, 56, 110, ..., 1202
        """
        if (layer_n - 2) % 6 != 0:
            raise ValueError('illegal layer_n: layer_n must be (natural number * 6 + 2) but given ', layer_n)

        n = (layer_n - 2) // 6
        channels = (16, 32, 64)
        output_map_size = ((32, 32), (16, 16), (8, 8))

        self.features = modules.Conv2d(
            in_channels=3, out_channels=channels[0], kernel_size=3, stride=1, padding=1)
